...
check_append_db <- function(d_table, table.name, sch = 'v1', constrains_db){
  ...
  if(any(colnames(d_table) %in% 'id')){
    ...
    if(db_n > 0) {
      ...
      dplyr::anti_join(
        d_table,
        dplyr::select(tbl_x(table.name, sch), col_unique),
        by = col_unique,
...
append_data <- function(d_table, table.name, sch = 'v1'){
  ...
  # upload the final table to the db
  get_new_data(d_table, constrains_db, sch, table.name) %>%
    append_data(., sch, table.name)
