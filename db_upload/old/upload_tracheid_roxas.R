...
#- FK
country_fk <- tbl(dbcon, 'country_fk') %>% dplyr::select(country, country_code) %>% collect() %>% deframe()
target_proxy_fk <- tbl(dbcon, 'target_proxy_fk') %>% dplyr::select(description, target_proxy) %>% collect() %>% deframe()
role_fk <- tbl(dbcon, 'role_fk') %>% dplyr::select(description, id) %>% collect() %>% deframe()
species_fk <- tbl(dbcon, 'species_fk') %>% dplyr::select(species, species_code) %>% collect() %>% deframe()
...
new.tree <- left_join( tree_info, tree.xls, by = c('xcel_name' = "TREE")) %>%
  dplyr::select(-xcel_name, -`Unit/remarks`) %>%
...
new.system <- left_join( measure_sys_info,  measure.xls, by = c('xcel_name' = 'Sample label') ) %>%
  dplyr::select(db_name, new.wood_sample$sample_label) %>%
...
roxas_output <- load_roxas_measurements(file_dir = file_dir, sample_id = new.measurement_info)
...
# 2.3.1 Persone role ------------------------------------------------------
tbl(dbcon, 'person') %>%
  inner_join(., new.person %>% dplyr::select(last_name, first_name, institution_code, person_role),  copy = T) %>%
  dplyr::select(person_id = id, role = person_role) %>%
  collect() %>%
...
# collect the tree id
tree.id <- tbl(dbcon, 'tree') %>% filter(site_id %in% site.id) %>% dplyr::select(tree_id = id, site_id, tree_label, species_code) %>% collect

# 2.5 Wood sample ---------------------------------------------------------
new.wood_sample %>% 
  inner_join(., tree.id %>% dplyr::select(tree_id, tree_label), by = 'tree_label') %>%
  dbWriteXcell(table.name = 'wood_sample', df = .)

# collect the wood_sample.id
wood_sample.id <- tbl(dbcon, 'wood_sample') %>% filter(tree_id %in% tree.id$tree_id) %>% dplyr::select(sample_id = id, tree_id, sample_label, radius, organ, cell_type) %>% collect

# 2.6 Measurement info ----------------------------------------------------
inner_join(new.measurement_info, new.system, by = 'sample_label') %>%
  inner_join(., dplyr::select(wood_sample.id, sample_label, sample_id), by = 'sample_label') %>%
...
measure_info.id <- tbl(dbcon, 'measure_info') %>% filter(sample_id %in% wood_sample.id$sample_id) %>% dplyr::select(subpiece_id = id, sample_id, data_filename, subpiece_label, system, software, software_version, magnification, editing_level) %>% collect

# 2.7 Ring information ----------------------------------------------------
roxas_output$roxas_annual_rings %>%
  inner_join(., dplyr::select(measure_info.id, data_filename, subpiece_id), by = 'data_filename') %>%
  mutate_all(funs(ifelse(. == -9999, NA, .))) %>%
  dbWriteXcell(table.name = 'ring', df = .)

ring.id <- tbl(dbcon, 'ring') %>% filter(subpiece_id %in% measure_info.id$subpiece_id) %>% dplyr::select(ring_id = id, subpiece_id, year) %>% collect
...
# 
# dim(temp)
# dbWriteXcell(table.name = 'tracheid_full', df = temp[1:500000,])
# dbWriteXcell(table.name = 'tracheid_full', df = temp[500001:1000000,])
# dbWriteXcell(table.name = 'tracheid_full', df = temp[1000001:1500000,])
# dbWriteXcell(table.name = 'tracheid_full', df = temp[1500001:2000000,])
# dbWriteXcell(table.name = 'tracheid_full', df = temp[2000001:2500000,])
# dbWriteXcell(table.name = 'tracheid_full', df = temp[2500001:3000000,])
# dbWriteXcell(table.name = 'tracheid_full', df = temp[3000001:3500000,])
# dbWriteXcell(table.name = 'tracheid_full', df = temp[3500001:4000000,])
# dbWriteXcell(table.name = 'tracheid_full', df = temp[4000001:4500000,])
# dbWriteXcell(table.name = 'tracheid_full', df = temp[4500001:nrow(temp),])
...
chelsa.summary<-ddply(chelsa, c("site_id","month","param"), summarize,
                      mean.value = mean(value))
ggplot(chelsa.summary, aes(x=month, mean.value, colour = as.factor(site_id))) +
  geom_line() +
  facet_grid(param ~ .)  +
  theme_bw()

cru.summary<-ddply(cru, c("site_id","month","param"), summarize,
                      mean.value = mean(value))
ggplot(cru.summary, aes(x=month, mean.value, colour = as.factor(site_id))) +
  geom_line() +
  facet_grid(param ~ .)  +
  theme_bw()

eu.summary<-ddply(eu, c("site_id","month","param"), summarize,
                   mean.value = mean(value))
ggplot(eu.summary, aes(x=month, mean.value, colour = as.factor(site_id))) +
  geom_line() +
  facet_grid(param ~ .)  +
  theme_bw()