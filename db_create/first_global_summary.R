...
#  filter(param_id %in% c(24, 25, 30, 31, 33, 37)) %>%
  ...
  right_join(
    tbl_x('cell') %>% 
     group_by(ring_id) %>%
     summarise(n_cells = n_distinct(paste0(x_cal,y_cal))), by =c('ring_id')
    ) %>% collect() %>% 
  inner_join(tbl_x('meas_param_fk') %>% select(param_id = id, parameter), by = 'param_id', copy=TRUE) %>%
  select(-param_id) %>%
