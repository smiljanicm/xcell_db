...
xray_output <- load_xray_measurements(file_dir = file_dir, sample_id = new.measurement_info)
...
# 2.3.1 Persone role ------------------------------------------------------
tbl(dbcon, 'person') %>%
  inner_join(., new.person %>% select(last_name, first_name, institution_code, person_role),  copy = T) %>%
...
inner_join(new.measurement_info, new.system, by = 'sample_label') %>%
  ...
  inner_join(., xray_output$xray_data_core, by = c("data_filename")) %>%
...
inner_join(ring.id, measure_info.id, by = 'subpiece_id') %>%
  ...
  inner_join(., xray_output$xray_data, by = c('data_filename', 'year')) %>%
  dbWriteXcell(table.name = 'profile', df = .)
...
# 
# dim(temp)
# dbWriteXcell(table.name = 'profile', df = temp[1:500000,])
# dbWriteXcell(table.name = 'profile', df = temp[500001:1000000,])
# dbWriteXcell(table.name = 'profile', df = temp[1000001:1500000,])
# dbWriteXcell(table.name = 'profile', df = temp[1500001:2000000,])
# dbWriteXcell(table.name = 'profile', df = temp[2000001:2500000,])
# dbWriteXcell(table.name = 'profile', df = temp[2500001:3000000,])
# dbWriteXcell(table.name = 'profile', df = temp[3000001:3500000,])
# dbWriteXcell(table.name = 'profile', df = temp[3500001:nrow(temp),])
# dbWriteXcell(table.name = 'profile', df = temp[4000001:4500000,])
# dbWriteXcell(table.name = 'profile', df = temp[4500001:nrow(temp),])
